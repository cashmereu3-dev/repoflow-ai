...
CREATE POLICY "Administrators can manage organizations" ON organizations
...
CREATE POLICY "Administrators and lenders can manage profiles" ON profiles
  FOR ALL USING (get_user_role() IN ('administrator', 'lender'));

-- ---- BORROWERS ----
CREATE POLICY "Org members can view borrowers" ON borrowers
  FOR SELECT USING (organization_id = get_user_org_id());

CREATE POLICY "Borrowers can view own record" ON borrowers
  FOR SELECT USING (profile_id = auth.uid());

CREATE POLICY "Lenders and managers can manage borrowers" ON borrowers
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );
...
CREATE POLICY "Lenders and managers can manage vehicles" ON vehicles
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );
...
CREATE POLICY "Managers can manage agents" ON agents
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );
...
CREATE POLICY "Borrowers can view own assignments" ON assignments
  FOR SELECT USING (
    get_user_role() = 'borrower' AND
    borrower_id IN (SELECT id FROM borrowers WHERE profile_id = auth.uid())
  );

CREATE POLICY "Agents can view assigned assignments" ON assignments
  FOR SELECT USING (
    get_user_role() = 'repo_agent' AND
    assigned_agent_id IN (SELECT id FROM agents WHERE profile_id = auth.uid())
  );

CREATE POLICY "Agents can update assigned assignments" ON assignments
  FOR UPDATE USING (
    organization_id = get_user_org_id() AND
    get_user_role() = 'repo_agent' AND
    assigned_agent_id IN (SELECT id FROM agents WHERE profile_id = auth.uid())
...
CREATE POLICY "Lenders and managers can manage assignments" ON assignments
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );
...
-- ---- UPLOADS ----
CREATE POLICY "Org members can view uploads" ON uploads
  FOR SELECT USING (organization_id = get_user_org_id());

CREATE POLICY "Agents can create uploads for their assignments" ON uploads
  FOR INSERT WITH CHECK (
    organization_id = get_user_org_id() AND
    uploaded_by = auth.uid()
  );

CREATE POLICY "Managers can manage uploads" ON uploads
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );
...
-- ---- RECOVERIES ----
CREATE POLICY "Org members can view recoveries" ON recoveries
  FOR SELECT USING (organization_id = get_user_org_id());

CREATE POLICY "Agents and managers can create recoveries" ON recoveries
  FOR INSERT WITH CHECK (organization_id = get_user_org_id());

CREATE POLICY "Managers can manage recoveries" ON recoveries
  FOR ALL USING (
    organization_id = get_user_org_id() AND
    get_user_role() IN ('administrator', 'lender', 'repo_manager')
  );

-- ---- AUDIT LOGS (Read-only for org members, no delete) ----
...
CREATE POLICY "Borrowers can view own payments" ON payments
  FOR SELECT USING (
    get_user_role() = 'borrower' AND
    borrower_id IN (SELECT id FROM borrowers WHERE profile_id = auth.uid())
  );

CREATE POLICY "Borrowers can create payment requests" ON payments
  FOR INSERT WITH CHECK (
...
-- ---- GPS EVENTS ----
CREATE POLICY "Org members can view GPS events" ON gps_events
  FOR SELECT USING (organization_id = get_user_org_id());

CREATE POLICY "Agents can create GPS events" ON gps_events
  FOR INSERT WITH CHECK (
    organization_id = get_user_org_id() AND
